...
describe("executeSync", () => {
  ...
  // ===== Cursor prune: drop stale entries for paths no longer in discovery =====

  it("should drop cursors and knownFilePaths entries for files that vanished from discovery", async () => {
    // Scenario: a path that was scanned in a prior sync (e.g. a Multica
    // codex-home/sessions/ symlink path now deduped, or a rotated log) lives
    // on in cursors.files / knownFilePaths forever, bloating cursors.json.
    // After this fix, every sync rebuilds both maps from current discovery.
    const claudeDir = join(dataDir, ".claude", "projects", "proj-prune");
    await mkdir(claudeDir, { recursive: true });
    ...
    // Inject a stale path (simulates pre-#154 Multica symlink path) into
    // both maps. It does not exist on disk, so discovery won't surface it.
    const cursorsPath = join(stateDir, "cursors.json");
    const cursorsData = JSON.parse(await readFile(cursorsPath, "utf-8"));
    ...
    cursorsData.knownFilePaths[stalePath] = true;
    await writeFile(cursorsPath, JSON.stringify(cursorsData));

    // Next sync — should prune the stale entry from both maps.
    await executeSync({
      stateDir,
      deviceId: "dev-1",
      claudeDir: join(dataDir, ".claude"),
    ...
    expect(after.knownFilePaths[stalePath]).toBeUndefined();
    // Real file's cursor must survive.
