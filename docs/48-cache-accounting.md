...
Overview retains the original annual Activity / Goal Tracker, statistics, Trends and Insights layout. The top desktop row also includes a salary calculator card; each annual heatmap is one horizontal strip of circular day cells, with native horizontal scrolling inside narrow cards. Usage summary groups Total Tokens, Cache Hit Rate and estimated cost into three aligned cards, retaining its section rule, accent bars and icons. Input/output totals, cache read/write totals with separate coverage, and monthly forecast/daily average sit inside their corresponding cards. Total Tokens and cost retain both full-period and elapsed-day week/month comparisons; the sign reflects the actual change, with lower cost shown in green. Net savings and detailed accounting remain available in the expandable details. The hit rate is token based: R / input with known read counts, weighted by that input across records. It is not an average of daily percentages or a fraction of requests. The daily hit-rate chart uses the same denominator. Mixed legacy and annotated records use the shared accounting helpers; uncollected reads show an unavailable rate, a measured zero stays 0%, and partial coverage is explicit. Cache is included in P and is never added to P + Q again.

The Overview page header owns the All Time / Last 6 Months / Last 3 Months / Last 1 Month selector. Month ranges begin on the same local date that many calendar months ago and include today; a missing date clamps to the target month's last day. Usage statistics, trends and insights share that period. Daily and half-hour queries receive identical UTC timestamps (`from` inclusive, `to` exclusive). All-time charts begin at the first recorded day. Other pages retain their existing calendar month/week controls. Activity and Goal Tracker retain the current-year scope and existing local goal settings; growth comparisons and the monthly forecast retain their calendar windows. The header's information button explains these exceptions and discloses approximate timing for the selected records in a keyboard- and touch-accessible popover. The salary card uses the selected period's estimated cost divided by its calendar days, including inactive days. Its gear button opens a large settings dialog with the existing ratio/price controls, explanation and detailed trend; adjustments update both the card and chart and survive closing the dialog. These presentation changes do not require an API or database migration.

Trends also pairs daily stacked token bars with a period-share donut. The model, harness and device selector changes both charts together and persists when the period changes. Both charts share one aggregation and the complete remainder as Other; missing calendar days are zero-filled. Models are selected by accounted tokens in the seven calendar days ending on the latest positive usage date within the selected period, then earlier seven-day windows until 30 models are selected. Ties use model names, so input ordering cannot change the selection. Token counts and share denominators still cover the entire selected period. The same selection rule applies to model evolution, device model trends and hourly model charts; Daily Usage preserves those 30 models plus Other in both its timeline and donut. Legends initially show five model entries, with the full list in an accessible popover; this never limits plotted series or recalculates shares. Harness and device selection remains based on the five largest period totals. Device charts use the actual device timeline with the same UTC query bounds, never an estimated split by model or harness. A failed device query offers retry while model and harness views remain available.
