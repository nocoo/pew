...
Trends also pairs daily stacked token bars with a period-share donut. The model, harness and device selector changes both charts together and persists when the period changes. Both charts share one aggregation and the complete remainder as Other; missing calendar days are zero-filled. Models are selected by accounted tokens in the seven calendar days ending on the latest positive usage date within the selected period, then earlier seven-day windows until five models are selected. Ties use model names, so input ordering cannot change the selection. Token counts and share denominators still cover the entire selected period. The same selection rule applies to model evolution, device model trends and hourly model charts; Daily Usage preserves those five models plus Other in both its timeline and donut. Harness and device selection remains based on the five largest period totals. Device charts use the actual device timeline with the same UTC query bounds, never an estimated split by model or harness. A failed device query offers retry while model and harness views remain available.
