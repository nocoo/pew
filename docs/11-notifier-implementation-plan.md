...
4. **blocking handoff**：拿锁失败的进程不 exit，而是 append signal 后阻塞等锁（blocking flock），拿到锁后自己跑 sync。这消除了 "check dirty → truncate → unlock" 窗口中的 lost wake-up
...
pew notify --source=xxx:
  1. 尝试 flock(sync.lock, LOCK_EX | LOCK_NB)
     ...
     └── 失败 (EAGAIN) → append "\n" 到 signal 确保 dirty
                        ...
                        → 拿到锁，跳到步骤 2
                        → 超时 → 返回 result with error，不跑 sync
...
| 场景 | 行为 |
|------|------|
| 首次 notify，无 sync 在跑 | 获取锁，截断 signal，执行 sync |
| sync 运行中，新 notify 到达 | 新进程获取锁失败 → append signal → 阻塞等锁 → 拿到锁后自己跑 sync |
| sync 完成，signal size > 0 | dirty follow-up，截断 signal，再跑一轮 |
| sync 完成，signal size == 0 | 释放锁，正常退出 |
| sync 运行中，N 个 notify 到达 | 所有都 append + 阻塞等锁；持锁者 follow-up 消化 dirty；等锁者拿到锁后发现 signal 为空则直接退出 |
...
| 阻塞等锁超时（lockTimeoutMs） | 返回 result with error，不跑 sync |
...
try {
  ...
  if (err.code === "EAGAIN" || err.code === "EWOULDBLOCK") {
    ...
    // 阻塞等锁（最多 lockTimeoutMs），拿到锁后自己跑 sync
    await withTimeout(fd.lock("exclusive"), lockTimeoutMs);
    // 拿到锁，继续正常流程（truncate → sync → check dirty）
  } else {
    // 其他错误 — 降级为无锁执行
  }
}
...
| 11 | 阻塞等锁超时 | 返回 result with error，executeSyncFn 不被调用 |
