...
- 参考 `packages/cli/src/cli.ts:224` 与 `:636`：这两处是**入口划分**，不是 token/session
  分工：
  - **`cli.ts:224`** = 手动 `pew sync` 命令入口。加载所有 usage + session opener，然后
    **分两次调用**：先 `executeSync({ ..., openZcodeDb })`（cli.ts:245 附近），再
    `executeSessionSync({ ..., openZcodeSessionDb })`（cli.ts:307 附近）——**每个函数
    只收自己需要的那个 opener**，交错传参会触发 TypeScript excess-property 错误。
  - **`cli.ts:636`** = `pew notify` 命令入口。装载 usage + session opener 全部，
    传给 `executeNotify`，notify 内部再各自转发给 executeSync / executeSessionSync。
  - 即两个入口 × 每个入口 2 个 opener 装载点 = 4 处动态 import。opener 变量各自
    对应的 executeXxx 函数不能混。

**解决**：给 zcode 增加一整条 opener 注入链路，与 OpenCode / Hermes 对齐：

1. **两个 opener 模块**（在 commit #3/#4 实现）：
...
5. `packages/cli/src/drivers/registry.ts`：
   ...
   - `SessionDriverRegistryOpts` 加 `zcodeDbPath?` + `openZcodeSessionDb?`；**独立**
     激活条件 `if (opts.zcodeDbPath && opts.openZcodeSessionDb)
     dbDrivers.push(createZcodeSessionDriver({...}))`——注意 session registry 判的是
     `openZcodeSessionDb`（session opener），不是 `openZcodeDb`（token opener）。
     若哥的运行环境只装载了两个 opener 之一（比如 session 侧的 dynamic import
     失败），那就只跑另一半 pipeline。
6. `packages/cli/src/cli.ts` **两个入口 × 每个入口两个 opener = 4 处动态 import**：
   - cli.ts:224 段（手动 sync 入口）**加两个** try/import：
     ...
     然后**分两次调用**：
     ```ts
     await executeSync({ ..., zcodeDbPath: paths.zcodeDbPath, openZcodeDb });
     await executeSessionSync({ ..., zcodeDbPath: paths.zcodeDbPath, openZcodeSessionDb });
     ```
     **不要**把 openZcodeSessionDb 塞给 executeSync（`SyncOptions` 没有这个字段，
     TS 会 excess-property 报错）；反之亦然。
...
**解决**：给 CLI `drivers/types.ts` 里的 `DbTokenResult` / `DbSessionResult`（不是 core；
DB driver result 是 CLI 私有类型）各加一个可选字段：
...
`ProgressEvent.phase`（`packages/cli/src/commands/sync.ts:80` 起的本地 interface，
既有代码里名字就叫 `ProgressEvent`）**已经**包含 `"warn"` 变体（见 sync.ts:82 的 union
和 10 处 `phase: "warn"` 用法）；`cli.ts` 里
...
| `outputTokens` | 由 §一挑战 6 的 provider-total 三分支决定：`reportedTotal == inclusiveOutput` → `max(0, output_tokens - reasoning_tokens)`；`reportedTotal == disjointOutput` → `output_tokens` 保留原值；两者都不匹配 → 采用 inclusive 分支 + `warn` |
...
opener 里 SQL 显式 `AS` 映射到 camelCase 列名，parser 拿到的 `ZcodeUsageRow` 直接
按 camelCase 属性访问，不再和 snake_case 混：
...
SELECT
  ...
  mu.provider_total_tokens         AS providerTotalTokens,   -- 可能为 NULL
  mu.computed_total_tokens         AS computedTotalTokens    -- NOT NULL default 0
FROM model_usage mu
...
Session opener 里同样 `AS`：`session.time_created AS timeCreated` /
`session.time_updated AS timeUpdated` / `session.task_type AS taskType`。
...
- **adapter 层测试要求**：opener 单元测试至少一条"in-memory sqlite ↔ ZcodeUsageRow
  往返 assert"用例——真造几行 `INSERT`，跑 opener SQL，断言拿到的 row 每个 camelCase
  字段类型 + 数值都对得上。防止仅靠 TypeScript 类型断言掩盖 `AS` 遗漏、拼错、类型
  不匹配（sqlite driver 出现 undefined 时 TS 编译不报，但 normalizer 里 `Math.max`
  会 NaN 或抛）。
...
let providerTotalMismatchWarned = false;   // 简单 bool 去重，避免每行 token 值文本
                                            // 都不同导致 seenWarn Set 失效

for (const row of rows) {
  const normalized = normalizeZcodeUsage(row);
  if (normalized.warn && !providerTotalMismatchWarned) {
    warnings.push(normalized.warn);
    providerTotalMismatchWarned = true;
  }
  // Drop terminal-but-zero rows (cancelled 或 error 但未产生 usage)。
  // 与 claude / grok / copilot-cli 等 parser 保持一致。
...
- **isAllZero 过滤**（review 追加项）：error/cancelled 行有可能 token 全 0（provider 未返回
  usage）。这些行进入 SQL 结果保证 cursor 前进（避免下次重扫），但**不生成**无意义
  QueueRecord。与 `packages/cli/src/parsers/claude.ts:124` 一致。
- **once-per-run 去重用 bool 而非 Set**：`normalizeZcodeUsage` 的 warn 文本内嵌
  `reportedTotal` 数值，每行不同 → Set 天然失效变成 per-row 报警。用 boolean
  只报一次；若未来需要多种 warn 类型，改成 `Set<string>` 并 hash warn code（不含数值）。
...
- `packages/cli/src/drivers/registry.ts`
  - `TokenDriverRegistryOpts` 加 `zcodeDbPath?: string` + `openZcodeDb?: (dbPath: string) => ...`；
    激活条件 `if (opts.zcodeDbPath && opts.openZcodeDb)`
  - `SessionDriverRegistryOpts` 加 `zcodeDbPath?: string` + `openZcodeSessionDb?: (dbPath: string) => ...`；
    **独立**激活条件 `if (opts.zcodeDbPath && opts.openZcodeSessionDb)`——判 session opener，
    不判 token opener
...
- `packages/cli/src/cli.ts`
  ...
  - **4 处动态 import**（review P1#4）——两个入口 × 每个入口两个 opener：
    - cli.ts:224 段（手动 `pew sync` 入口）：**加两组** try/import，一组 `openZcodeDb`
      给 `executeSync`（cli.ts:245），一组 `openZcodeSessionDb` 给 `executeSessionSync`
      （cli.ts:307）——不要把 session opener 塞给 executeSync
    - cli.ts:636 段（`pew notify` 入口）：**镜像同样两组** try/import，全部传给
...
- API `VALID_SOURCES` **6 处 allowlist**（4 处 `?source=` query + 2 处 body 里 `alias.source`）：
  - Query allowlist（`?source=`）：
  ...
  - Body allowlist（`aliases[].source`）：
    - `packages/web/src/app/api/projects/route.ts:15`（POST /api/projects，body 里
      `aliases[].source`）
    - `packages/web/src/app/api/projects/[id]/route.ts`（PATCH，body 里 `add_aliases[].source` /
      `remove_aliases[].source`）
...
| 12 | `provider_total` 与 `inclusiveOutput` 和 `disjointOutput` **两者都不匹配** | 归一化结果照常返回，`normalized.warn` 非空；parser 汇总只 emit 1 条（bool 去重） |
| 12b | terminal 但 tokens 全 0（`status='cancelled'` 且 provider 未返回 usage） | 该 row 不 push 进 deltas；cursor 仍推进（避免下次重扫）；rowCount +1 |
...
2. **4 处 query allowlist** + **2 处 body allowlist**，全部走一遍：
   - Query（返回 200 with results）：
     - `/api/usage?source=zcode&...`
     - `/api/sessions?source=zcode`
     - `/api/leaderboard?source=zcode&...`
     - `/api/users/[slug]?source=zcode&...`
   - Body：
     - POST `/api/projects` with `aliases: [{source: "zcode", project_ref: ...}]`
       （session-alias-only 通道，201 with 扁平 project）
     - PATCH `/api/projects/[id]` with `add_aliases: [{source: "zcode", ...}]`（200）
...
2. 若 zcode 仍把 cache_creation 算进 `input_tokens`（当前 CLI 源码分支）：pew 现有归一化
   天然正确
3. 若 zcode 未来把 cache_read / cache_creation 从 `input_tokens` 里拆出去（"cache 变
   disjoint"）：`normalizeZcodeUsage` **不会自动切换**。当前 `disjointOutput` 只对
   reasoning 与 output 的关系建模（`+ reasoning`），没有对 cache 关系建模，所以
   provider_total 会同时命中 inclusive 和 disjoint 都不匹配 → 进入 warn 分支。
   届时需要**专门升级 input 归一化**：给 normalizer 加第二维检测（e.g. 观察若
   `provider_total ≈ (input + cache_read + cache_creation + output)` 则切 disjoint-input
   模式），并加一整套 test 锁定新语义。**不能声称"自动切换"**。
   同时永远**不要**用 `input_tokens < cr + cc` 判断——inclusive 模式下 input 完全
   可能仍 > cache 之和（本机 4 行样本就是这种情况）。判断口径只能靠 `provider_total` 关系。
