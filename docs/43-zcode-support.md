...
4. **cache_creation 目前视为 input 的一部分（不重复计）**：本机 4 条样本 `cache_creation=0`。
   保守设计：入 `input` bucket、不额外 emit，直到看到非零样本再评估是否扩 `TokenDelta`。
   决策详见 §1.4 与 §三。
...
**关键决策：cache_creation 处理**

- **本机数据不足**：4 条样本 cache_creation 都是 0，无法直接验证 pew 的公式。
- **CLI 源码显示 cache_creation 已在 `input_tokens` 内**（`input_tokens > 0` 时不重复加）。
- **保守做法**：把 `input_tokens - cache_read_input_tokens - cache_creation_input_tokens`
  作为 pew `TokenDelta.inputTokens`；把 `cache_read + cache_creation` 合并成
  `cachedInputTokens`。这样 `total_tokens` 归一后仍等于 `input + output`，与 zcode
  `computed_total` 一致，也不会因为 cache_creation 非零而重复计数。
- **风险与回退**：若未来出现 `input_tokens = 0 && (cache_creation + cache_read) > 0` 的
  样本（zcode fallback 分支），pew 也会正确：`inputDisjoint = max(0, 0 - cr - cc) = 0`，
  `cachedInputTokens = cr + cc`，`total = 0 + (cr+cc) + output + 0`，与 zcode
  `computed_total = (cr+cc) + output` 完全一致。
- **不扩 core `TokenDelta`**：pew 现有 4 字段（input / cached / output / reasoning）
  能够无损承载 zcode 数据，扩 5 字段会引发 Core / Queue / D1 schema / RPC / Web / Worker
  的全链路 migration，不成比例。
...
| pew `TokenDelta` 字段 | 来源公式 |
|---|---|
| `inputTokens` | `max(0, input_tokens - cache_read_input_tokens - cache_creation_input_tokens)` |
| `cachedInputTokens` | `cache_read_input_tokens + cache_creation_input_tokens` |
| `outputTokens` | `max(0, output_tokens - reasoning_tokens)` |
...
export function normalizeZcodeUsage(raw: {
  ...
  const cached = raw.cacheReadInputTokens + raw.cacheCreationInputTokens;
  return {
    inputTokens: Math.max(0, raw.inputTokens - cached),
...
**5.4.a Orchestrator dispatch 抽象**（review P0#1）：

- `packages/cli/src/commands/sync.ts`
  - 抽象 DB cursor 路由表（`opencode / hermes / zcode`）；删除 `isOpenCode / isHermes`
    硬编码分支
  - `dbsScanned` 字段扩容 or 改成 `Partial<Record<Source, number>>`
  - `sourceKey()` switch 加 `case "zcode": return "zcode"`
  - `SyncResult.sources.zcode` / `filesScanned.zcode` 初始化 0
  - `knownDbSources` backfill 段加 `zcodeSqlite` 分支
  - driver loop 加 `try/catch`，保证 driver 级异常隔离
- `packages/cli/src/commands/session-sync.ts`
  - 同上抽象：删除对 `cursors.openCodeSqlite` 的直接赋值；改按 driver.source 路由
  - `sourceCounts` / `dbsScanned` 加 zcode 列
  - driver loop `try/catch`
- `packages/cli/src/commands/session-sync-helpers.ts` — `sourceKey()` 加 `case "zcode"`
- `packages/cli/src/notifier/coordinator.ts` — `sourceKey()` 加 `case "zcode"`；
  `SyncCycleResult.sources.zcode` 初始化

**5.4.b Driver 注册与发现**：

- `packages/cli/src/drivers/types.ts` — `DiscoverOpts` 加 `zcodeDbPath?: string`
- `packages/cli/src/discovery/sources.ts` — `discoverZcodeDb()`：文件存在返回 `[path]`
  否则 `[]`
- `packages/cli/src/drivers/registry.ts` — 注册 token + session 两个 driver（`zcodeDbPath`
  存在时激活）；registry test 覆盖两个激活分支
...
- ~~`packages/cli/src/storage/prune-alias-cursors.ts`~~ — **不动**。prune 只处理文件 cursor
  （见 `pruneAliasCursors` 签名），DB cursor 由 orchestrator 自己管理，走不同 codepath。
...
- `packages/cli/src/cli.ts`
  ...
  - 3 处 `DiscoverOpts` 构造点传入 `zcodeDbPath: paths.zcodeDbPath`
    （sync / session-sync / notify → 参见 cli.ts:264 / 321 / 679）
  - sync 完成消息里 `if (result.sources.zcode > 0) deltaParts.push(`ZCode: ${...}`)`
...
- `README.md` — "12 种" → "13 种"；banner + 特性表 + 命令表加 ZCode（按字母序，介于 Pi 与
  PM Studio 之间，或位置视既有字母序而定）；"无 hook" 列表加 ZCode
- `packages/web/src/components/landing/landing-content.tsx` — hero source 清单加 ZCode
  （字母序）；"无 hook" 说明加 ZCode
...
**依赖顺序**：`sourceKey()` 用 `never` 穷举检查（sync.ts / session-sync.ts /
session-sync-helpers.ts / notifier/coordinator.ts），任何 commit 只要给 `Source`
加 `"zcode"`，四处 switch 立刻编译失败；driver 依赖 `DiscoverOpts.zcodeDbPath` 必须
同一或更早 commit 存在。为此重排：

| # | Commit | 内容 | 独立编译? |
|---|---|---|---|
| 1 | **`feat: add "zcode" source foundation (types + exhaustive switches + DiscoverOpts stub)`** | `core/types.ts` 加 `zcode` + 两个 cursor interface；`core/constants.ts` `SOURCES`；4 个 `sourceKey()` 加 `case "zcode"`；`SyncResult/SyncCycleResult.sources.zcode` 初始化；`drivers/types.ts` `DiscoverOpts.zcodeDbPath?`；`utils/paths.ts` 加 zcodeHome/zcodeDbPath；`cli.ts` `isSource()` + `SOURCE_LABELS`；core & CLI 相关 tests（constants/types/validation/paths）。**尚无 parser / driver / dispatch 改动** | ✅ |
...
| 3 | `feat(cli): add zcode sqlite parser + normalizer (no wiring)` | `parsers/zcode-sqlite.ts` + `zcode-sqlite-db.ts` + `parsers/__tests__/zcode-sqlite.test.ts`。TDD 先测后码 | ✅ |
| 4 | `feat(cli): add zcode sqlite session parser (no wiring)` | `parsers/zcode-session.ts` + test | ✅ |
| 5 | `feat(cli): add zcode token+session drivers and register them` | `drivers/token/zcode-sqlite-token-driver.ts`、`drivers/session/zcode-sqlite-session-driver.ts`、`discovery/sources.ts` `discoverZcodeDb()`、`drivers/registry.ts` 注册；registry test 覆盖激活分支 | ✅ |
| 6 | `feat(cli): wire zcode through sync + session-sync + notify` | 3 处 `DiscoverOpts` 传入 `zcodeDbPath`；sync/session-sync/status test 追加 zcode 覆盖。此时 CLI 端 zcode 完整可用 | ✅ |
...
| 8 | `docs(43): mark zcode support as implemented; update CLAUDE.md/README/landing/docs-index` | 4 个文档 + doc 43 status → done + 12 → 13 | ✅ |
...
| 9 | `reasoning_tokens > 0`（构造） | outputDisjoint=`out - rea`；reasoning 字段透传；`input + output + cached + reasoning == computed_total` 不变式 |
| 10 | `cache_creation > 0`（构造） | cachedInputTokens=`cache_read + cache_creation`；inputDisjoint=`input - cached` |
| 11 | `input_tokens = 0 && cache_creation + cache_read > 0`（zcode fallback 分支） | inputDisjoint=0；cached 保留全量；total 与 zcode `computed_total = (cr+cc) + out` 一致 |
| 12 | `provider_total_tokens != computed_total_tokens`（构造不一致） | log warn（可选）；采用 parser 归一化结果 |
...
| Case | 期望 |
|---|---|
| DiscoverOpts 无 zcodeDbPath | driver 不激活 |
| zcodeDbPath 指向不存在文件 | driver 激活但 discover 返回 []，run 返回空 |
| zcodeDbPath 指向真实 db.sqlite | cursor 语义与 OpenCode driver 一致 |
...
**Q3：为什么不用 `TokenTracker`（本地参考 B）的 `message` 表 JSON 路径？**
...
- 本机 zcode CLI：`~/.zcode/cli/db/db.sqlite`（schema 0.15.0）
