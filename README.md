...
│  VS Code Copilot · GitHub Copilot CLI · Hermes Agent             │
└───────────────┬──────────────────────────────────────────────────┘
...
- **8 种 AI 工具** — Claude Code、Codex CLI、Gemini CLI、OpenCode、OpenClaw、VS Code Copilot、GitHub Copilot CLI、Hermes Agent
...
- **通知钩子** — `pew init` 一键安装，AI 工具会话结束后自动触发同步（支持 7 种工具；Hermes 需手动安装插件）
